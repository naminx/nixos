{
  ...
  my-chromium = import ./chromium/default.nix pkgs;
  my-mcomix = import ./mcomix.nix pkgs;
...
in {
  ...
  programs = {
    ...
    wezterm.enable = true;
    wezterm.extraConfig = pkgs.lib.strings.fileContents ./xdg/wezterm.lua;
    # better cd (z)
    zoxide.enable = true;
  ...
  xdg = {
    configFile = with pkgs.lib.strings; {
      ...
      "google-chrome/NativeMessagingHosts/com.google.chrome.remote_webauthn.json".source = "${pkgs.chrome-remote-desktop}/etc/opt/chrome/native-messaging-hosts/com.google.chrome.remote_webauthn.json";
